const { getInstance } = require("../config/RazorpayLazy");
...
exports.capturePayment = async(req, res)=>{
    ...
    console.log("capturePayment body:", req.body);
console.log("capturePayment user:", req.user);
    ...
    // create Order
    try{
        const razor = getInstance();
        if(!razor){
            console.error("Razorpay instance not initialized — check RAZORPAY_KEY/SECRET");
            return res.status(500).json({ success: false, message: "Razorpay not configured" });
        }
        const paymentResponse = await razor.orders.create(options);
