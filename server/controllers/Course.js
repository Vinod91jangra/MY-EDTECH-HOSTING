...
exports.createCourse = async(req,res)=>{
    try{
        ...
        // Accept either `tag` or `tags` from frontend and normalize to `tag`
        const rawTag = _tag || _tags;
        const tag = rawTag ? JSON.parse(rawTag) : [];
        ...
        for(const key in updates){
            if(updates.hasOwnProperty(key)){
                if(key === "tag" || key === "instructions"){
                    ...
                    // normalize `tags` to the `tag` field in the schema
...
exports.getSpecificCourseDetails = async(req, res)=>{
    try{
        ...
        if(!courseDetails || courseDetails.length){
            return res.status(400).json({
                success: false,
                message: `Could not find course with id: ${courssId}`,
            });
        }
